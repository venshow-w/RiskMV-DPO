...
from magicdrivedit.datasets import *
from magicdrivedit.registry import DATASETS, build_module
from magicdrivedit.mmdet_plugin.datasets.pipelines.loading_utils import one_hot_encode, one_hot_decode
...
def save_key_in_h5(h5: h5py.File, key_dtype, token, data):
    for key in key_dtype.keys():
        ...
        if key_dtype[key] == np.uint8:
            ...
            # TODO: data can be saved as uint8 rather than int32.
...
@hydra.main(version_base=None, config_path="../../configs", config_name="cache_gen")
def main(cfg: DictConfig):
    ...
    if cfg.process == "train":
        dataset = build_module(
            OmegaConf.to_container(cfg.dataset.data.train, resolve=True), DATASETS,
        )
    elif cfg.process == "val":
        dataset = build_module(
            OmegaConf.to_container(cfg.dataset.data.val, resolve=True), DATASETS,
        )
    else:
        print("nothing to do")
        return
    ...
    with h5py.File(f"{cfg.process}_{cfg.subfix}.h5", "w") as h5:
        ...
        for batch in tqdm(loader):
            for data in batch:
                for datai in data:
                    if isinstance(datai, str):
                        logging.warning(f'Error real token: {datai}')
                        continue
                    token = datai["metas"].data["token"]
                    token = token.replace("/", ";")
                    save_key_in_h5(h5, KEYS2SAVE, token, datai)
