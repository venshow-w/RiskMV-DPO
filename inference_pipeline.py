...
def main():
    ...
    #     print("\n" + "="*20)
    #     print("Rank 0 进入调试模式 (pdb)")
    #     print("输入 'n' 下一步, 'p 变量名' 查看变量, 'c' 继续执行")
    #     print("="*20 + "\n")
    #     import pdb; pdb.set_trace()  # 只有主进程停在这里
    # else:
    #     # 2. 其他进程（Rank 1）打印一条消息，然后继续执行
    #     # 注意：它们会在后续需要同步的地方（如 dist.init_process_group）等待 Rank 0
    #     print(f"Rank {os.environ.get('RANK')} 正在运行中，等待主进程同步...")
    ...
    if is_distributed():
        ...
        dist.init_process_group(
            ...
            init_method="tcp://localhost:12366")
        cfg.sp_size = 1
    ...
    with tqdm(
        ...
        for i, batch in pbar:
                ...
                if is_main_process():
                    for idx, batch_prompt in enumerate(batch_prompts):
                        ...
                        save_path = save_sample(
                            ...
                            split_6views_for_image=cfg.get("split_6views_for_image", False),
                        )
                del video_clips
                coordinator.block_all()
            time8 = time()
            ...
            if is_main_process() and not cfg.ignore_ori_imgs:
                print(f"  - VAE 解码 & 图片保存: {time8 - time7:.2f}s")
                print(f"  - 本轮迭代总计: {time8 - t_iter_start:.2f}s\n")
                torch.cuda.empty_cache()
            ...
            t_iter_end = time()
            if is_main_process():
                # 使用 \r 和 sys.stdout.flush() 确保在 tqdm 刷新时也能看到
                import sys
                msg = (
                    f"\n\n"
                    f"DEBUG TIMER - Iteration {i}:\n"
                    # f"  [1. 数据读取与前处理]: {t_prep_duration:.2f}s\n"
                    # f"  [2. DiT模型采样(30步)]: {t_sample_duration:.2f}s\n"
                    f"  [3. VAE解码与保存图片]: {t_iter_end - time7:.2f}s\n"
                    f"  [TOTAL 迭代总时长]: {t_iter_end - t_iter_start:.2f}s\n"
                    f"{'-'*40}\n"
                )
    ...
    coordinator.destroy()
    time9 = time()
    print('config time', time2-time1)
    print('load data time', time3-time2)
    print('load model time', time4-time3)
    print('build scheduler time', time5-time4)
    print('prep infer time', time6-time5)
    print('infer model time', time7-time6)
    print('save sample time', time8-time7)
    print('save gt time', time9-time8)
    print('all infer model time', time8-time5)
