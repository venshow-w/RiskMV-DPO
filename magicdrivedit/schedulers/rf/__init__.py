...
from .rectified_flow import timestep_transform
from .rflow_with_pred import RFlowSchedulerWithPred as RFlowScheduler
...
class RFLOW:
    ...
    def training_losses(self, model, x_start, model_kwargs=None, noise=None, mask=None, weights=None, t=None,return_x0_pred=None, return_features=False,  # 新增：是否返回中间特征
                       feature_layers=None):
        return self.scheduler.training_losses(model, x_start, model_kwargs, noise, mask, weights, t, return_x0_pred=return_x0_pred, return_features=return_features,  # 新增：是否返回中间特征
    ...
    def sample(
        ...
        for i, t in progress_wrap(enumerate(timesteps)):
            ...
            _model_args = replace_with_null_condition(
                ...
                ["y", "bbox", "cams", "rel_pos", "maps", 'first_frame_latent', 'frames_mask', 'first_frame_images'], append=False)
            if mask is not None:
                _model_args["x_mask"] = mask_t_upper
            pred = model(z, t, **_model_args)
            if pred.shape[1] == z.shape[1] * 2:
