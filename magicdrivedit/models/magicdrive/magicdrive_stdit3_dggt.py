...
from .crosstransformers3d import Projector

from dggt.models.vggt import VGGT


class VGGTCrossAttentionBlock(nn.Module):
    """
    Cross attention block to fuse VGGT features with the main diffusion features.
    This block is initialized with zero weights to ensure the pretrained model
    behavior is preserved initially.
    """
    def __init__(
        self,
        hidden_size,
        ...
        # Layer normalization for query (main features)
        self.norm_q = get_layernorm(
            hidden_size, eps=1e-6, affine=False, use_kernel=enable_layernorm_kernel
        )
        
        # Layer normalization for key-value (VGGT features)
        self.norm_kv = get_layernorm(
            hidden_size, eps=1e-6, affine=False, use_kernel=enable_layernorm_kernel
        )
        ...
        # Output projection - initialized to zero for stble training
        self.output_proj = zero_module(nn.Linear(hidden_size, hidden_size))
    ...
    def forward(self, x, vggt_latent, t_emb, NC, T, S):
        """
        Args:
            x: Main diffusion features [B, T*S, C]
            vggt_latent: VGGT encoded features [B, N_vggt, C]
            t_emb: Time embedding for modulation [B, 2*C]
            T: Number of temporal frames
            S: Number of spatial patches
        
        Returns:
            x: Fused features [B, T*S, C]
        """
        B_NC, N, C = x.shape
        B = B_NC // NC
        
        # Get modulation parameters
        shift, scale = (self.scale_shift_table[None] + t_emb.reshape(B, 2, -1)).chunk(2, dim=1)
        shift = repeat(shift, "b 1 c -> (b nc) 1 c", nc=NC)
        scale = repeat(scale, "b 1 c -> (b nc) 1 c", nc=NC)
        
        # Normalize and modulate query
        x_norm = t2i_modulate(self.norm_q(x), shift, scale)
        
        # Normalize key-value (VGGT features)
        vggt_norm = self.norm_kv(vggt_latent)
        
        # Cross attention: x as query, vggt_latent as key-value
        attn_out = self.cross_attn(x_norm, vggt_norm, mask=None)
...
class VGGTFusionBlockV2(nn.Module):
    """
    VGGT融合模块 V2版本 - 更激进的设计
    
    相比V1的改进:
    1. 使用6个参数的scale_shift_table (与MultiViewSTDiT3Block主分支一致)
    2. 添加独立的MLP分支处理VGGT特征
    3. 更强的特征对齐能力
    """
    ...
    def __init__(
        ...
        for m in self.vggt_proj.modules():
            if isinstance(m, nn.Linear):
                nn.init.xavier_uniform_(m.weight, gain=0.1) # 0.02
        ...
        # ============ MLP分支 ============
        from timm.models.vision_transformer import Mlp
        from magicdrivedit.models.layers.blocks import approx_gelu
        self.mlp = Mlp(
        ...
        # ============ 输出投影 ============
        self.attn_proj = zero_module(nn.Linear(hidden_size, hidden_size))
        self.mlp_proj = zero_module(nn.Linear(hidden_size, hidden_size))
    ...
    def forward(
        ...
        (shift_attn, scale_attn, gate_attn, 
         shift_mlp, scale_mlp, gate_mlp) = repeat(
            self.scale_shift_table[None] + t.reshape(B, 6, -1),
            "b ... -> (b NC) ...", NC=NC,
        ).chunk(6, dim=1)
        
        if x_mask is not None:
            (shift_attn_zero, scale_attn_zero, gate_attn_zero,
        ...
        # ============ Cross Attention分支 ============
        x_v = t2i_modulate(self.norm_q(x), shift_attn, scale_attn)
        if x_mask is not None:
            x_v_zero = t2i_modulate(self.norm_q(x), shift_attn_zero, scale_attn_zero)
            x_v = self.t_mask_select(x_mask, x_v, x_v_zero, T, S)
        
        vggt_projected = self.vggt_proj(vggt_latent)
        vggt_kv = self.norm_kv(vggt_projected)
        # vggt_kv = repeat(vggt_kv, "b n c -> (b nc) n c", nc=NC)
        
        vggt_attn_output = self.cross_attn(x_v, vggt_kv, mask=None)
        
        # Gate调制
        x_attn_s = gate_attn * vggt_attn_output
        if x_mask is not None:
            x_attn_s_zero = gate_attn_zero * vggt_attn_output
            x_attn_s = self.t_mask_select(x_mask, x_attn_s, x_attn_s_zero, T, S)
        
        # 残差连接
        x_attn_s = self.attn_proj(self.drop_path(x_attn_s))
        x = x + x_attn_s
        
        # ============ MLP分支 ============
        x_m = t2i_modulate(self.norm_mlp(x), shift_mlp, scale_mlp)
        if x_mask is not None:
            x_m_zero = t2i_modulate(self.norm_mlp(x), shift_mlp_zero, scale_mlp_zero)
        ...
        # 残差连接
        x_m_s = self.mlp_proj(self.drop_path(x_m_s))
        x = x + x_m_s
...
class MagicDriveSTDiT3Config(PretrainedConfig):
    ...
    def __init__(
        ...
        ### VGGT相关参数
        use_vggt=True,
        vggt_checkpoint=None,
        freeze_vggt=True,
        vggt_fusion_layers=[1,3,5,8,11,15,20,26],
        vggt_feature_dim=3072,
        only_train_vggt_proj_and_vggt_fusion_block=True, 
        **kwargs,
    ):
        ...
        self.only_train_vggt_proj_and_vggt_fusion_block = only_train_vggt_proj_and_vggt_fusion_block
        self.vggt_fusion_layers = vggt_fusion_layers
        self.vggt_feature_dim = vggt_feature_dim
...
class MagicDriveSTDiT3(PreTrainedModel):
    ...
    def __init__(self, config: MagicDriveSTDiT3Config):
        ...
        self.freeze_vggt = config.freeze_vggt
        self.only_train_vggt_proj_and_vggt_fusion_block = config.only_train_vggt_proj_and_vggt_fusion_block
        if self.use_vggt:
            self.vggt = VGGT()
            # VGGT特征投影层（将VGGT输出投影到hidden_size维度） 假设VGGT输出维度为1024，需要根据实际情况调整
            self.vggt_proj = Projector(
                in_channels=config.vggt_feature_dim, 
                embed_dim=self.hidden_size
                )
            # 时间embedding的额外分支（用于VGGT cross attention的调制）
            self.vggt_t_block = nn.Sequential(
                nn.SiLU(),
                nn.Linear(self.hidden_size, 2 * self.hidden_size, bias=True)
            )
            # 可学习的null embedding (用于CFG)
            self.vggt_null_embedding = nn.Parameter(
                torch.zeros(1, 1, self.hidden_size)
            )
            # VGGT融合层位置
            self.vggt_fusion_layers = config.vggt_fusion_layers
            # 为每个融合层创建cross attention block
            self.vggt_fusion_blocks = nn.ModuleDict({
                str(layer_idx): VGGTFusionBlockV2(   #VGGTCrossAttentionBlock(
                ...
                for layer_idx in self.vggt_fusion_layers
            })
        ...
        # 新增：首帧引导的可学习缩放因子,初始化为 0.5 
        # self.first_frame_scale = nn.Parameter(torch.full((1,), 0.5))

        ##############################用已训练好的模型权重初始化#####################
        self.initialize_weights()

        # ===== 加载VGGT checkpoint =====
        if self.use_vggt and config.vggt_checkpoint is not None:
            self._load_vggt_checkpoint(config.vggt_checkpoint)
     
        if self.only_train_vggt_proj_and_vggt_fusion_block:
            self._set_training_status_freeze_old(config)
        else:
            self._set_training_status_all(config)
        # breakpoint()
    ...
    def _set_training_status_freeze_old(self, config):
        """设置各模块的训练状态（冻结/解冻） freeze other only train vggt projector and fusion module"""
        # set training status
        if self.use_vggt:
            if config.freeze_vggt:
                for param in self.vggt.parameters():
                    param.requires_grad = False
                logging.info("VGGT parameters frozen")

            # VGGT投影层可训练
            for param in self.vggt_proj.parameters():
                param.requires_grad = True
            # VGGT时间embedding可训练
            for param in self.vggt_t_block.parameters(): ###########################################
                param.requires_grad = True
            # VGGT null embedding可训练
            self.vggt_null_embedding.requires_grad = True
            for block in self.vggt_fusion_blocks.values():
                for param in block.parameters():
                    param.requires_grad = True
            logging.info("VGGT fusion modules are trainable")
        
        # 原有的freeze逻辑
        if config.freeze_y_embedder or self.only_train_vggt_proj_and_vggt_fusion_block:
            for param in self.y_embedder.parameters():
                param.requires_grad = False
        if config.freeze_x_embedder or self.only_train_vggt_proj_and_vggt_fusion_block:
            for param in self.x_embedder.parameters():
                param.requires_grad = False
        if config.freeze_old_embedder or self.only_train_vggt_proj_and_vggt_fusion_block:
            ...
            for param in self.fps_embedder.parameters():
                param.requires_grad = False
        if config.freeze_temporal_blocks or self.only_train_vggt_proj_and_vggt_fusion_block:
        ...
        if config.freeze_old_params or self.only_train_vggt_proj_and_vggt_fusion_block:
            for param in self.parameters():
                param.requires_grad = False
        ...
        # make sure all new parameters require grad 确保新增模块可训练
        # cross view attn
        for block in self.base_blocks_s:
            if hasattr(block, "cross_view_attn"):
                ...
                for param in block.mva_proj.parameters():
                    param.requires_grad = False
                block.scale_shift_table_mva.requires_grad = False
    ...
    def _set_training_status_all(self, config):
        """设置各模块的训练状态（冻结/解冻）"""
    ...
    def initialize_weights(self):
        ...
        if self.use_vggt:
            # 初始化投影层
            for module in self.vggt_proj.modules():
                _zero_init(module)
            # 初始化时间embedding
            for block in self.vggt_t_block.modules():
                if isinstance(module, nn.Linear):
                    nn.init.xavier_uniform_(module.weight)
                    if module.bias is not None:
                        nn.init.zeros_(module.bias)
            
            # null embedding初始化为0 (已经是0了)
            nn.init.zeros_(self.vggt_null_embedding)
    ...
    def extract_vggt_features(self, first_frame_images, drop_cond_mask=None):
        """
        Extract VGGT features from first frame images.
        
        Args:
            first_frame_images: Tensor of shape [B, T, NC, C, H, W] - first frame RGB images
            drop_cond_mask: Tensor of shape [B] - CFG mask (0=drop, 1=keep)
        
        Returns:
            vggt_features: Tensor of shape [B*NC, N, C] - extracted features
        """
        if not self.use_vggt:
            return None
        
        with torch.no_grad() if self.freeze_vggt else torch.enable_grad():
            # VGGT expects images in shape [B, NC, C, H, W] where NC is number of cameras
            # For simplicity, we assume first_frame_images already has the right format
            first_frame_images = rearrange(first_frame_images, "B T NC C ... -> B (NC T) C ...")
            
            aggregated_tokens_list, image_tokens_list, dino_token_list, image_feature, patch_start_idx = self.vggt.aggregator(first_frame_images)
            # aggregated_tokens_list shape [torch.Size([1, 6, 1925, 2048])]*24
            # image_tokens_list shape [torch.Size([1, 6, 1925, 3072])]*24
            # dino_token_list shape [torch.Size([1, 6, 1925, 1024])]*24
            # image_feature shape torch.Size([1, 6, 32, 60, 1024])
            # Extract relevant features (e.g., world_points, depth, gs_map, etc.)
            # Here we use image_feature as the main feature
        ...
        # Assuming features is [B, N, vggt_feature_dim]
        vggt_features = self.vggt_proj(features)
        if vggt_features.max()==0:
            # print('===================================================')
            breakpoint()
            print(f"VGGT features - mean:{vggt_features.mean():.8f}, std:{vggt_features.std():.8f}, max:{vggt_features.abs().max():.8f}")

        # ===== CFG处理: 对需要drop的样本使用零向量 =====
        if drop_cond_mask is not None:
            # drop_cond_mask: [B] where 0 means drop, 1 means keep
            # 扩展到匹配vggt_features的维度
            B = first_frame_images.shape[0]
            NC = vggt_features.shape[0] // B  # 从特征中推断NC
            
            # 创建null特征（可学习或零向量）
            if hasattr(self, 'vggt_null_embedding'):
                null_features = self.vggt_null_embedding.expand_as(vggt_features)
            else:
                null_features = torch.zeros_like(vggt_features)
            # 或者使用可学习的null embedding:
            # null_features = self.vggt_null_embedding.expand_as(vggt_features)
            
            # 扩展mask到正确形状
            mask = repeat(drop_cond_mask, "b -> (b nc) 1 1", nc=NC)
            vggt_features = mask * vggt_features + (1 - mask) * null_features
            #features = rearrange(features, "(b nc) P C -> b nc P C", nc=NC)
    ...
    def forward(self, x, timestep, y, maps, bbox, cams, rel_pos, fps,
                ...
                first_frame_latent=None, frames_mask=None, first_frame_images=None ,**kwargs):
        """
        Forward pass of MagicDrive.
        VGGT融合位置说明：
        - 放在 base_blocks_s (spatial attention) 之后
        - 放在 base_blocks_t (temporal attention) 之前
        - 这样可以让几何信息先融入空间特征，再传播到时间维度
        """
    
        # x shape(2,96,5,106,200) first_frame_latent shape (2, 96, 1, 106,200)
        dtype = self.x_embedder.proj.weight.dtype
        B, real_T = x.size(0), rel_pos.size(1)
        NC = len(mv_order_map) if mv_order_map is not None else 1
        ...
        vggt_latent = None
        if self.use_vggt and first_frame_images is not None:
            vggt_latent = self.extract_vggt_features(first_frame_images, drop_cond_mask)
            _, ref_T, ref_NC,_, _, _ = first_frame_images.shape
            vggt_latent = rearrange(vggt_latent, "(B T) S C -> B (T S) C", T=ref_T)
        ...
        # # VGGT time embedding
        # vggt_t_mlp = None
        # if self.use_vggt and vggt_latent is not None:
        #     vggt_t_mlp = self.vggt_t_block(t)

        # === condition embeddings (text, bbox, camera, frame) ===
        ...
        #     first_frame_latent = rearrange(first_frame_latent, "B (C NC) T ... -> (B NC) C T ...", NC=NC)
        #     f_tokens = self.x_embedder(first_frame_latent)
        #     # x_b = torch.cat([f_tokens, x_b], dim=1)
        #     # l = f_tokens.shape[1]
        #     # x_b[:, :l] = x_b[:, :l] +  f_tokens # self.first_frame_scale *
        #     num_f0_tokens = f_tokens.shape[1]
        #     injection_mask = repeat(drop_cond_mask, "b -> (b nc) 1 1", nc=NC)
        #     x_b_f0_guided = f_tokens
        #     x_b[:, :num_f0_tokens] = (injection_mask * x_b_f0_guided + (1.0 - injection_mask) * x_b[:, :num_f0_tokens]) 
        ...
        # breakpoint()
        # =========================================================================
        # Transformer Blocks (Control Depth部分: 0 ~ control_depth-1)
        # =========================================================================
        for block_i in range(0, self.control_depth):  
            ...
            ## VGGT fusion ##
            if self.use_vggt and vggt_latent is not None and block_i in self.vggt_fusion_layers:
                fusion_block = self.vggt_fusion_blocks[str(block_i)]
                
                # x = fusion_block(x, vggt_latent, vggt_t_mlp, NC, T, S)
                x = fusion_block(
                        x=x,
                        vggt_latent=vggt_latent,
                        t= t_mlp,  # 使用完整的时间嵌入
                        t0=t0_mlp if x_mask is not None else None,
                        x_mask=x_mask,
                        T=T,
                        S=S,
                        NC=NC,
...
def load_from_pixart_pretrained(model: MagicDriveSTDiT3, pretrained):
    ...
    (m, u) = model.load_state_dict(base_model.state_dict(), strict=False)
    if model.x_control_embedder is not None:
        model.x_control_embedder.load_state_dict(base_modsel.x_embedder.state_dict())
    _m, _u = [], []
    for key in m:
        if key.startswith("base_blocks_") or key.startswith("control_blocks_"):
            pass
...
@MODELS.register_module("MagicDriveSTDiT3-XL/2-dggt")
def MagicDriveSTDiT3_XL_2_dggt(from_pretrained=None, force_huggingface=False, **kwargs):
