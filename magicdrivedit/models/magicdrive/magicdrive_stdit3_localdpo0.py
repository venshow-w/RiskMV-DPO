...
class MagicDriveSTDiT3(PreTrainedModel):
    ...
    def extract_geometry_latents(self, first_frame_images, drop_cond_mask=None):
        ...
        输入: [B, NC, C, H, W]
        输出: [B, num_geo_tokens, hidden_size]
        """
        if not self.use_vggt_adapter or self.geometry_adapter is None:
            return None
        
        # if drop_cond_mask is not None:
        #     if drop_cond_mask.dim() == 2:  # [B, T]
        #         drop_cond_mask = drop_cond_mask[:, 0] 
    ...
    def get_intermediate_features(self, x, t=None, layer_indices=None):
        ...
        with torch.no_grad():  # 对齐损失不需要梯度
            for i, block in enumerate(self.base_blocks_s):
                # 前向传播（简化，实际需要完整参数）
                h = block(h, ...)  # 这里需要根据实际block的forward调整
                
                if i in layer_indices:
                    # 保存该层特征
                    feat = h.clone()  # [B, N, D]
                    # 空间池化
                    layer_features[f'layer_{i}'] = feat.mean(dim=1)  # [B, D]
