...
@PIPELINES.register_module()
class LoadMultiViewDepthFromFiles:
    """从独立的文件列表中加载多视图深度图。
    
    期望 results['depth_paths'] 是一个文件名列表。

    Args:
        to_float32 (bool): 是否转换为 float32。默认为 True。
        load_format (str): 加载格式，'png' 或 'npz'。默认为 'png'。
        scale_factor (float): 深度值的缩放系数。默认为 1.0。
    """

    def __init__(self, to_float32=True, load_format="png", scale_factor=1.0):
        self.to_float32 = to_float32
    ...
    def __call__(self, results):
        ...
        Args:
            results (dict): 包含 'depth_paths' 的字典。

        Returns:
            dict: 增加 'depth' 键，值为 List[PIL.Image] 或 List[np.ndarray]。
        """
        depth_filenames = results["depth_paths"]
        depth_maps = []
        
        for name in depth_filenames:
            if self.load_format == "png":
                # 使用 PIL 加载。对于 16-bit 深度图，'I;16' 格式会被正确保留
                depth = Image.open(name)
                # 如果需要转换成 np.ndarray 并处理
                # depth = np.array(depth).astype(np.float32) * self.scale_factor
                depth_maps.append(depth)
            elif self.load_format == "npz" or self.load_format == "npy":
                # 针对 DepthAnything 或你之前脚本保存的 npz/npy 格式
                if os.path.exists(name):
                    data = np.load(name)
                    # 如果是 npz，可能需要指定 key，如 data['depth']
                    depth = data if isinstance(data, np.ndarray) else data['depth']
                    if self.to_float32:
                        depth = depth.astype(np.float32)
                    depth = depth * self.scale_factor
                    depth_maps.append(Image.fromarray(depth)) # 转回 PIL 保持与 RGB 代码一致

        results["depth_filename"] = depth_filenames
        # 将深度图存入结果字典
        results["depthmap"] = depth_maps
        # 记录深度图形状
        results["depth_shape"] = depth_maps[0].size  if len(depth_maps)>0 else None # (W, H)
        
        # 为了适配后续的 DiT 分支，通常需要将深度图也进行 padding 对齐
        # 如果 RGB 代码里有 padding，这里也必须同步执行
        results["depth_ori_shape"] = depth_maps[0].size  if len(depth_maps)>0 else None
        
        return results

    def __repr__(self):
        return f"{self.__class__.__name__}(to_float32={self.to_float32}, scale_factor={self.scale_factor})"
