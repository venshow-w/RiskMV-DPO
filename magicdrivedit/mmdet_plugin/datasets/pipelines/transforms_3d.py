...
from ...core import VoxelGenerator
...
@PIPELINES.register_module()
class ImageDepthAug3D:
    """3D-aware 图像和深度图增强。
    同时对 RGB 和 Depth 应用完全一致的变换。
    """
    ...
    def img_transform(
        self, img, rotation, translation, resize, resize_dims, crop, flip, rotate, is_depth=False
    ):
        # adjust image
        # 对于深度图，如果使用 PIL，resize 建议使用 NEAREST 或 BILINEAR
        resample_mode = Image.BILINEAR if not is_depth else Image.BILINEAR
        
        img = img.resize(resize_dims, resample=resample_mode)
        img = img.crop(crop)
        if flip:
            img = img.transpose(method=Image.FLIP_LEFT_RIGHT)
        
        # 注意：rotate 对深度图的操作需要谨慎，通常使用 BILINEAR 即可
        img = img.rotate(rotate, resample=resample_mode)

        # 这里的矩阵更新逻辑只与几何坐标系有关，RGB 和 Depth 共享一套
        # 以下代码保持原样...
        rotation *= resize
        translation -= torch.Tensor(crop[:2])
        if flip:
    ...
    def __call__(self, data: Dict[str, Any]) -> Dict[str, Any]:
        imgs = data["img"]
        depths = data.get("depthmap", None) # 获取深度图列表
        ...
        for i in range(len(imgs)):
            ...
            # 处理 RGB
            new_img, rotation, translation = self.img_transform(
                img, post_rot, post_tran, resize, resize_dims, crop, flip, rotate, is_depth=False
            )
            new_imgs.append(new_img)
            
            # 同步处理 Depth (使用完全相同的参数)
            if depths is not None and len(depths)>0:
                depth = depths[i]
                new_depth, _, _ = self.img_transform(
                    depth, post_rot.clone(), post_tran.clone(), resize, resize_dims, crop, flip, rotate, is_depth=True
                )
                new_depths.append(new_depth)

            # 更新变换矩阵 (RGB/Depth 共享同一个 img_aug_matrix)
            transform = torch.eye(4)
            transform[:2, :2] = rotation
            transform[:2, 3] = translation
            transforms.append(transform.numpy())
...
@PIPELINES.register_module()
class GridMask:
    """Apply GridMask directly to image."""
...
@PIPELINES.register_module()
class RandomFlip3D:
    def __init__(self, random=True, always_v=False, always_h=False) -> None:
        self.random = random
        self.always_v = always_v
        self.always_h = always_h
...
class ObjectPaste:
    ...
    def __call__(self, data):
        ...
        data["points"] = points
        # in original logic, there is no need to handle this. we just drop here.
        data.pop("visibility")
