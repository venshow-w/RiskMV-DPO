...
class DefaultFormatBundle3D:
    ...
    def __call__(self, results):
        ...
        # 2. 新增：处理深度图
        if "depthmap" in results:
            if len(results["depthmap"])>0:
                depth_tensors = []
                for d in results["depthmap"]:
                    # 将 PIL Image 或 np.ndarray 转换为 tensor
                    # to_tensor 会处理 HWC -> CHW 的转换
                    d_tensor = to_tensor(np.array(d))
                    
                    # 确保深度图有通道维度 (1, H, W)
                    if d_tensor.ndim == 2:
                        d_tensor = d_tensor.unsqueeze(0)
                    elif d_tensor.ndim == 3 and d_tensor.shape[0] != 1:
                        # 如果不小心加载成了 (H, W, 1) 被转成 (1, H, W) 之外的情况，强制校准
                        d_tensor = d_tensor[0:1, ...] 
                    
                    depth_tensors.append(d_tensor)
            
                # 将多视图深度图堆叠为 (N, 1, H, W)
                results["depthmap"] = DC(torch.stack(depth_tensors), stack=True) 
            else:
                # results.pop('depthmap')
                results["depthmap"] = None
