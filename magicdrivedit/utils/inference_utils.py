...
from magicdrivedit.datasets import IMG_FPS
...
def replace_with_null_condition(_model_args, uncond_cam, uncond_rel_pos,
    ...
    if 'first_frame_images' in keys and 'first_frame_images' in _model_args:
        handled_keys.append("first_frame_images")
        first_frame_images = _model_args["first_frame_images"]
        null_first_frame_images = torch.zeros_like(first_frame_images)
        if append:
            model_args['first_frame_images'] = torch.cat([first_frame_images, null_first_frame_images], dim=0)
        else:
            model_args['null_first_frame_images'] = null_first_frame_images
        keys.remove("null_first_frame_images")
