...
def collate_fn_single_clip(
    ...
    global2camera = torch.stack([
        torch.from_numpy(example["global2cameras"]).float() for example in examples], dim=0)
    ...
    ret_dict = {
        ...
        "camera_param_raw": {
            "int": camera_int,
            "ext": camera_ext, # lidar2camera
            "aug": camera_aug,
        },
        "global2camera" : global2camera,
    }
...
class NuScenesTvalDataset(NuScenesDataset):
    def __init__(
        self,
        ann_file,
        pipeline=None,
        ...
        **kwargs,
        ...
        self.infos_from_planner = None
        if info_file_from_planner is not None:
            self.infos_from_planner = json.load(open(info_file_from_planner, 'r', encoding='utf-8'))   
            logging.info(f"Loaded planner info for token: {self.infos_from_planner.get('token')}")
        ...
        self.drop_first_frames = drop_first_frames
    ...
    def load_annotations(self, ann_file):
        ...
        """利用 scene_tokens 结构进行过滤"""
        data = mmcv.load(ann_file)
        all_infos = data["infos"]
        scene_tokens_list = data['scene_tokens'] 
        # breakpoint()
        
        if self.infos_from_planner is not None:
            target_frame_token = self.infos_from_planner.get('token')
            
            target_allowed_tokens = set()
            new_scene_tokens_list = []
            
            # 找到目标场景
            for seq in scene_tokens_list:
                if target_frame_token in seq:
                    target_allowed_tokens = set(seq)
                    new_scene_tokens_list.append(seq)
                    break 
            
            if target_allowed_tokens:
                logging.info(f"Planner filtering: Keeping {len(target_allowed_tokens)} frames.")
                # 仅保留属于该场景的帧信息
                all_infos = [info for info in all_infos if info['token'] in target_allowed_tokens]
                scene_tokens_list = new_scene_tokens_list
            else:
                logging.warning(f"Token {target_frame_token} not found in {ann_file}!")

        # 排序
        self.data_infos = list(sorted(all_infos, key=lambda e: e["timestamp"]))
        # 应用 load_interval (通常验证集是 1)
        ...
        # 核心修复点：重新建立索引映射
        # build_clips 内部会使用 self.token_data_dict = {item['token']: idx ...}
        # 这一步会根据当前的 self.data_infos 重新映射，保证索引从 0 开始
        self.clip_infos = self.build_clips(self.data_infos, scene_tokens_list)
        
        logging.info(f"After filtering, dataset has {len(self.clip_infos)} clips.")
    ...
    def load_clip(self, clip):
        ...
        for frame_idx in clip:
            ...
            # 注入 JSON 中的 Planner 数据
            if self.infos_from_planner is not None and info['token'] == self.infos_from_planner.get('token'):
                # 覆盖关键字段
                for key in ['gt_boxes', 'gt_names', 'gt_velocity', 'ego2global_translation', 'ego2global_rotation']:
                    if key in self.infos_from_planner:
                        # 确保转为 numpy 格式                      
                        info[key] = np.array(self.infos_from_planner[key])
                
                # 特别处理 can_bus (如果有)
                if 'can_bus' in self.infos_from_planner:
                    info['can_bus'] = self.infos_from_planner['can_bus']
            
            # 获取处理后的 frame_info (包含图像路径等)
            frame_info = super().get_data_info(frame_idx)
            
            # 由于 info 已被覆盖，这里的 obtain_next2top 会计算正确的相对位姿
    ...
    def load_frames(self, frames):
        ...
        ret_dicts['first_frames'] = ret_dicts['pixel_values'][:1,]
