...
def save_sample(x, save_path=None, fps=8, normalize=True, value_range=(-1, 1), force_video=False, high_quality=False, verbose=True, with_postfix=True, force_image=False, split_6views_for_image=False, save_per_n_frame=-1):
    """
    Args:
    ...
    if not force_video and x.shape[1] == 1:  # T = 1: save as image
        ...
        if force_image:         
            for xi, _x in enumerate(imgList):
                if split_6views_for_image:
                    cams=['CAM_FRONT_LEFT', 'CAM_FRONT', 'CAM_FRONT_RIGHT', 'CAM_BACK_LEFT', 'CAM_BACK', 'CAM_BACK_RIGHT']
                    h, w = _x.shape[:2]
                    sh, sw = int(h/2), int(w/3)
                    _x_views = []
                    for i in range(2):
                        for j in range(3):
                            _x_views.append(_x[i*sh:(i+1)*sh, j*sw:(j+1)*sw] )
                    for ind, cam in enumerate(cams):
                        _save_path_ = os.path.join(save_path.replace('.mp4', ''), f"f{xi:05d}", cam+'.png')
                        os.makedirs(os.path.dirname(_save_path_), exist_ok=True)
                        Image.fromarray(_x_views[ind]).save(_save_path_)
                else:
                    _save_path_ = os.path.join(save_path.replace('.mp4', ''), f"f{xi:05d}.png")
                    os.makedirs(os.path.dirname(_save_path_), exist_ok=True)
                    Image.fromarray(_x).save(_save_path_)
        ...
        if high_quality:
            ...
            if save_per_n_frame > 0 and len(imgList) > save_per_n_frame:
                ...
                clip.write_videofile(save_path, verbose=verbose, bitrate="4M",fps=12, logger='bar' if verbose else None)
                clip.close()
        else:
