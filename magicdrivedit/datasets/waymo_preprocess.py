...
class WaymoProcessor:
    ...
    #     # --- 修改后的视频生成核心循环 ---
    #     target_w, target_h = 240, 160

    #     for frame_id in trange(num_frames, desc=f"Making video {scene_id}", dynamic_ncols=True):
    #         frame_cams = []
    #         for cam_id in [3, 1, 0, 2, 4]:
    #     # 1. 读取并缩放原始图像
    #             image_path = f"{scene_path}/images_{downsample_factor}/{str(frame_id).zfill(3)}_{str(cam_id)}.jpg"
    #             image_pil = Image.open(image_path).convert("RGB").resize((target_w, target_h))
    #             img_array = np.array(image_pil)

    #     # 2. 处理深度图可视化
    #             depth_flows_path = f"{scene_path}/depth_flows_{downsample_factor}/{str(frame_id).zfill(3)}_{str(cam_id)}.npy"
    #             depth_flows = np.load(depth_flows_path)
    #             depth_image = depth_flows[:, :, 0]
        
    #     # 获取可视化数组并强制转换为 uint8
    #             depth_img_arr = depth_visualizer(depth_image, depth_image > 0)
    #             depth_img_arr = self._to_uint8(depth_img_arr) # 辅助函数见下方
    #             depth_pil = Image.fromarray(depth_img_arr).resize((target_w, target_h))

    #     # 3. 处理流场图可视化
    #             flow_image = depth_flows[:, :, 1:4]
    #             flow_img_arr = scene_flow_to_rgb(flow_image, flow_max_radius=15)
    #             flow_img_arr = self._to_uint8(flow_img_arr)
    #             flow_pil = Image.fromarray(flow_img_arr).resize((target_w, target_h))

    #     # 4. 处理地面掩码叠加 (保持逻辑一致)
    #             ground_path = image_path.replace("images", "ground_label").replace("jpg", "png")
    #             if os.path.exists(ground_path):
    #                 g_mask = Image.open(ground_path).resize((target_w, target_h)).convert("L")
    #                 g_arr = np.array(g_mask) / 255.0
    #                 g_arr = np.where(g_arr > 0.5, 1, 0)[..., np.newaxis]
            
    #                 img_float = img_array / 255.0
    #                 img_float = img_float * (1 - g_arr) + g_arr * 0.3 # 叠加 30% 灰色
    #                 img_array = (img_float * 255).astype(np.uint8)

    #     # 5. 垂直拼接
    #             column = np.concatenate([img_array, np.array(depth_pil), np.array(flow_pil)], axis=0)
    #             frame_cams.append(column)

    # # 6. 水平拼接所有相机
    #         full_frame = np.concatenate(frame_cams, axis=1)
    ...
    def _to_uint8(self, arr):
        """将任何数值类型的数组安全转换为 uint8 RGB 格式"""
        arr = np.array(arr)
    # 如果是浮点型且最大值 <= 1.0，认为是 0-1 归一化数据
