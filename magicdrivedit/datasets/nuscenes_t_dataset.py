...
def collate_fn_single_clip(
    ...
    if 'depthmap' in examples[0] and examples[0]['depthmap'] is not None:
        depthmaps =  torch.stack([example["depthmap"].data for example in examples])
        depthmaps = depthmaps.to(memory_format=torch.contiguous_format).float()
        ret_dict['depthmap'] = depthmaps
    # else:
        # ret_dict['depthmap'] = None
