...
from worldcache.cache_3D import Cache4D
from worldcache.camera_utils import generate_camera_trajectory
from worldcache.moge_depth_predictor import _predict_moge_depth_from_tensor
...
def main():
    ...
    with tqdm(
        ...
        for i, batch in pbar:
            ...
            first_frames =  batch.pop("first_frames")
            ...
            ### ======= world cache =========
            intrinsics_b33 = batch['camera_param_raw']["int"][:, 0, :,:,:]
            initial_w2c_b44 = batch['camera_param_raw']["ext"]
            augmatrix = batch['camera_param_raw']["aug"]
            global2cameras = batch['global2camera'][:, 0, :,:,:]
            first_frames_ = first_frames[:,0,:,:,:, :]
         
            moge_model = MoGeModel.from_pretrained("Ruicheng/moge-vitl").to(device)
            pred_depth, pred_mask = _predict_moge_depth_from_tensor(
                first_frames_, moge_model
            )

            cache = Cache4D(
                input_image=first_frames_.clone(), # [B, C, H, W]
                input_depth=pred_depth,       # [B, 1, H, W]
                input_mask=pred_mask,         # [B, 1, H, W]
                input_w2c=global2cameras,  # [B, 4, 4]
                input_intrinsics=intrinsics_b33,# [B, 3, 3]
                filter_points_threshold=0.05,
                input_format=["B", "F", "C", "H", "W"],
                foreground_masking=True,
            )

            generated_w2cs, generated_intrinsics = generate_camera_trajectory(
                trajectory_type='zoom_in',
                initial_w2c=global2cameras,
            ...
            # == model input format ==
            model_args = {}
            model_args['first_frames'] = first_frames
