...
def main():
    # 设定预计算特征目录
    precomputed_dir = "pwd/media/omnisky/12dd907f-8a2c-4a49-954c-a33edc979c06/PublicDatasets/nuscenes/MagicDriveDiT-nuScenes-metadata/prep_latent_424-800"
    # ======================================================
        ...
        with tqdm(
            ...
            for step, batch in pbar:
                ...
                with timers["move_data"] as move_data_t:
                    ...
                    depthmap = batch.pop('depthmap').to(device, dtype)
                    breakpoint()
                ...
                #     # 随机屏蔽首帧引导
                #     model_args["first_frame_latent"] = None #torch.zeros_like(model_args["first_frame_latent"])
                # else:
                #     # model_args['first_frame_latent'] = x[:,:,:1,:,:] #([2, 96, 1, 106, 200])
                model_args['first_frame_latent'] = first_frame_latent
                model_args['frames_mask'][:,:,:first_frame_latent.shape[2]]=1
                mask[:, :first_frame_latent.shape[2]]=False
                ...
                with timers["backward"] as backward_t:
                    loss = loss_dict["loss"].mean().to(torch.float32)
