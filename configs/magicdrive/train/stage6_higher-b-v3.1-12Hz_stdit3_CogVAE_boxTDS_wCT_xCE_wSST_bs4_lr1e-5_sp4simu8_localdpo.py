...
model = dict(
    type= "MagicDriveSTDiT3-XL/2-localdpo", # "MagicDriveSTDiT3-XL/2-dggt",
    ...
    # freeze_old_params = True,
    # freeze_old_embedder =True,
    # freeze_temporal_blocks = True,
    # magicdrive
    ...
    # ==================== ✅ 新增：VGGT几何适配器（Gen3R风格） ====================
    use_vggt_adapter=True,           # 新增：使用几何适配器，不是Cross Attention
    vggt_checkpoint="/media/omnisky/12dd907f-8a2c-4a49-954c-a33edc979c06/pretrained/dggt/pretrained/model_latest_waymo.pt",
    freeze_vggt=True,
    vggt_feat_dim=3072,
    num_geo_tokens=16,               # 几何latent的token数
...
partial_load = '/mnt/projects/MagicDrive-V2/outputs/MagicDriveSTDiT3-XL-2_stage3_448x840-v3.1-12Hz_stdit3_CogVAE_boxTDS_wCT_xCE_wSST_bs4_lr1e-5_sp4simu8_20260225-1916/epoch0-global_step5000'
...
# load = '/mnt/projects/MagicDrive-V2/outputs/MagicDriveSTDiT3-XL-2-localdpo_stage6_higher-b-v3.1-12Hz_stdit3_CogVAE_boxTDS_wCT_xCE_wSST_bs4_lr1e-5_sp4simu8_localdpo_20260214-2357/epoch0-global_step5000'
...
# load='/mnt/projects/MagicDrive-V2/outputs/MagicDriveSTDiT3-XL-2-localdpo_stage6_higher-b-v3.1-12Hz_stdit3_CogVAE_boxTDS_wCT_xCE_wSST_bs4_lr1e-5_sp4simu8_localdpo_20260217-1944/epoch0-global_step10000'
# load = '/mnt/projects/MagicDrive-V2/outputs/MagicDriveSTDiT3-XL-2-localdpo_stage6_higher-b-v3.1-12Hz_stdit3_CogVAE_boxTDS_wCT_xCE_wSST_bs4_lr1e-5_sp4simu8_localdpo_20260222-2120/epoch0-global_step17500'
load = None
grad_clip = 1.0
# lr = 4e-5 # 8e-5
ema_decay = 0.99
...
lambda_sft = 0.1  
lambda_align = 0.01
align_type = 'kl'    # 'cosine', 'kl', 'contrastive'
alpha_l = 0.1
alpha_h = 0.9
temperature=0.1
num_timesteps = 1000
...
# learning_rate = 1e-4  # 为新增模块设置学习率
# ✅ 运动感知遮罩参数（核心创新）
motion_aware_mask = True
mask_size_range = (0.1, 0.25)
speed_range = (0.05, 0.12)
motion_threshold = 0.7  # 光流阈值，高于此值认为是运动区域
use_semantic_prior = True  # 使用VGGT语义先验

# 几何适配器参数
num_geo_tokens = 16
geo_adapter_layers = 2

# ✅ 效率报告参数（ECCV 2026 Efficient Vision赛道）
report_compute_efficiency = True
gpu_hours_baseline = 10752  # MagicDrive-V2官方训练成本（32卡 × 14天 × 24小时）
gpu_hours_ours = 240        # 您的方案训练成本（2卡 × 5天 × 24小时）

# ==================== 4. 优化器配置修改 ====================
# 降低学习率，因为只训练新增模块
lr = 2e-5  # 从4e-5降低，防止破坏预训练特征
warmup_steps = 50  # 减少warmup，新增模块收敛快